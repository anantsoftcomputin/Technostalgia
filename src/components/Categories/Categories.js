      ...
      try {
        ...
        if (response?.data?.results?.length > 0) {
          ...
          const digitalDetoxCategory = response.data.results.find(
            (cat) => cat.slug === "communication-device"
          );
  ...
  useEffect(() => {
    const fetchCategoryBlogs = async () => {
      try {
        ...
        setLoading(true);
        const response = await ajaxCall(
          `/posts-category/?site_domain=techsnostalgia.com&category_slug=${selectedCategory.slug}`,
          { method: "GET" }
        );
                  ...
                  >
                    <div className="relative h-48">
                      ...
                      <div className="absolute top-4 left-4">
                        <span
                          className="px-3 py-1 rounded-full bg-white/90 text-teal-600 text-sm font-medium"
