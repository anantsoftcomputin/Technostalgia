      ...
      try {
        ...
        if (filtered.length > 0) {
          ...
          if (matchedTag) {
            ...
            setTagDescription(
              `Discover engaging articles about ${matchedTag.name} on Techsnostalgia. Explore vintage technology, retro gadgets, classic games, and the evolution of ${matchedTag.name}. Perfect for tech lovers, collectors, and anyone passionate about nostalgic tech and digital history.`
            );
          }
        }
      } catch (error) {
        ...
        {loading ? (
                ...
                >
                  ...
                  <div className="p-4 sm:p-6 flex flex-col flex-grow">
                        ...
                        {blog.tags &&
                          blog.tags.slice(0, 3).map((tag, index) => (
                            <Link
                              key={index}
                              href={`/tag/${tag.slug}`}
                              className="inline-flex items-center gap-1 px-2 py-1 rounded-full border-2 border-teal-100 bg-teal-100 text-teal-600 text-sm"
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-teal-600 text-sm sm:text-base font-medium hover:text-teal-700 transition-colors">
                        Read More →
                      </span>
                    </div>
                  </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              {searchQuery
                ? `No results found for "${searchQuery}" in ${tagName}`
                : `No articles available for ${tagName} yet.`}
            </p>
            <button
              onClick={() => setSearchQuery("")}
